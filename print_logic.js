...
function exportPrintableList() {
    const attendingEvents = [];
    const allEvents = [...appData];
    ...
    const allVoyageDates = new Set(appData.map(d => d.date));
    ...
    sortedDates.forEach(date => {
        ...
        const portName = portNotes[date] ? ` <span style="font-weight:normal; font-size: 0.9em; color: #4b5563;">(${portNotes[date]})</span>` : '';
            ...
            if (allInstances.length > 0) {
                ...
                if (isEveryDay) {
                    ...
                    allInstances.forEach(sib => {
                        ...
                        const isAttendingSib = attendingIds.has(sib.uid);
...
function exportPrintableGrid() {
    ...
    allEvents.forEach(ev => {
        ...
        if (attendingIds.has(uid)) attendingEvents.push({ ...ev, startMins: s, endMins: timeData.end + SHIFT_END_ADD, uid: uid, isHiddenTemp: isHidden });
    });
    ...
    customEvents.forEach(ev => {
        ...
        if (attendingIds.has(ev.uid)) attendingEvents.push({ ...ev, isHiddenTemp: isHidden });
    });
    ...
    sortedDates.forEach(date => {
        ...
        const portName = portNotes[date] ? ` <span style="font-weight:normal; color:#666;">(${portNotes[date]})</span>` : '';
